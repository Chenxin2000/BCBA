...
word_dir = "I:/corpora/"
word_name = "encrypted_burst0413.txt"
...
def split_cap(pcap_path, pcap_file, pcap_name, pcap_label='', dataset_level='packet'):
...
def get_feature_flow(label_pcap, payload_len, payload_pac):
    ...
    feature_result = extract(label_pcap, filter='tcp')
...
def generation(pcap_path, samples, features, splitcap=False, payload_length=128, payload_packet=5,
               dataset_save_path="E:\\ex_resultsNonvpn\\", dataset_level="packet"):
    ...
    for parent, dirs, files in os.walk(pcap_path):
        ...
        tls13 = 0
        if tls13:
            record_file = "E:\\ex_resultsNonvpn\\picked_file_record"
            target_path = "E:\\ex_resultsNonvpn\\packet_splitcap\\"
        ...
        for dir in label_name_list:
            for p, dd, ff in os.walk(parent + "\\" + dir):
                ...
                if splitcap:
                    for file in ff:
                        session_path = (split_cap(pcap_path, p + "\\" + file, file.split(".")[-2], dir,
    ...
    for index in range(len(label_name_list)):
        print("%s\t%s\t%d" % (
        label_id[label_name_list[index]], label_name_list[index], dataset[label_id[label_name_list[index]]]["samples"]))
...
if __name__ == '__main__':
    ...
    #pcap_path = "I:\\pcaps\\"
    # tls 13 downstream
    pcap_path, samples, features = "E:\\nonvpn\\pcap\\", 500, ["payload","length","time","direction","message_type"]
    X,Y = generation(pcap_path, samples, features, splitcap=True)
    # pretrain data
