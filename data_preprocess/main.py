...
_category = 6 # dataset class
dataset_dir = "E:\\nonvpn\\" # the path to save dataset for dine-tuning

pcap_path, dataset_save_path, samples, features, dataset_level = "E:\\nonvpn\\pcap\\splitcap\\", "E:\\ex_resultsNonvpn\\", [5000], ["payload"], "packet"
#数据集提取功能
def dataset_extract(model):
    
    X_dataset = {}
    Y_dataset = {}
#检查现有数据集：此部分检查指定目录中是否已存在数据集。如果是，它会加载数据集并在使用函数处理后返回X_dataset和。Y_datasetmodels_deal，如果该目录不存在或存在异常，它将继续生成新的数据集。
    try:
...
#生成新数据集：如果没有现有数据集，它将使用dataset_generation.generation具有指定参数的函数生成一个新数据集。
