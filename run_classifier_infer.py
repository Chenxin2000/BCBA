...
class Classifier(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = str2encoder[args.encoder](args)  # 假设这是一个类似于BERT的编码器
        self.labels_num = args.labels_num
        self.hidden_size = args.hidden_size
        self.soft_targets = args.soft_targets

        # CNN层
        self.conv = nn.Conv1d(in_channels=self.hidden_size, out_channels=self.hidden_size, kernel_size=3, padding=1)
        self.relu = nn.ReLU()

        # BiLSTM层
        self.bilstm = nn.LSTM(input_size=self.hidden_size, hidden_size=self.hidden_size // 2, num_layers=1,
                              batch_first=True, bidirectional=True)

        # 自注意力机制层
        self.self_attention = nn.MultiheadAttention(embed_dim=self.hidden_size, num_heads=12)

        # 全连接层
        self.fc1 = nn.Linear(self.hidden_size, self.hidden_size)  # 调整输入大小
        self.dropout = nn.Dropout(0.7)  # 正则化的dropout层
        self.output_layer = nn.Linear(self.hidden_size, self.labels_num)

    def forward(self, src, tgt, seg, soft_tgt=None):
        emb = self.embedding(src, seg)
        emb = emb.transpose(1, 2)  # 准备进行卷积

        # CNN处理
        cnn_output = self.conv(emb)
        cnn_output = self.relu(cnn_output)
        cnn_output = cnn_output.transpose(1, 2)  # 形状：[batch, seq_len, hidden_size]

        # BiLSTM处理
        lstm_output, _ = self.bilstm(cnn_output)  # 形状：[batch, seq_len, hidden_size]

        # 自注意力机制处理
        attention_input = lstm_output.transpose(0, 1)  # 形状调整为：[seq_len, batch, features]
        attention_output, _ = self.self_attention(attention_input, attention_input, attention_input)
        attention_output = attention_output.transpose(0, 1)  # 形状：[batch, seq_len, hidden_size]

        # 序列长度上的池化（假设使用最大池化）
        pooled_output = torch.max(attention_output, dim=1)[0]  # 形状：[batch, hidden_size]
        ...
        if tgt is not None:
            if self.soft_targets and soft_tgt is not None:
                soft_loss = nn.MSELoss()(logits, soft_tgt)
                hard_loss = nn.NLLLoss()(nn.LogSoftmax(dim=-1)(logits), tgt.view(-1))
...
def evaluate(args, dataset, print_confusion_matrix=False):
    ...
    if print_confusion_matrix:
        ...
        with open("data2/results/matrixCBA.txt",'w') as f:
...
def main():
    ...
    model.load_state_dict(torch.load('models/pretrained_model.bin'), strict=False)
